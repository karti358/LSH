...
def generate_sig_matrix(sig_nrows,sig_ncols, r_matrix):
    ...
    idxs = np.arange(start = 0, stop = r_nrows, dtype = np.int32)

    perms = None
    for perm in range(sig_nrows):
        temp_perm = np.argsort(np.random.permutation(idxs))[...,np.newaxis]
        if perms is None: perms = temp_perm
        else: perms = np.concatenate([perms, temp_perm], axis = 1)

    sig_matrix = np.zeros(shape = (sig_nrows, sig_ncols), dtype = np.int32)
    for k in range(sig_nrows): 
        temp = r_matrix[perms[:,k], :]

        for col in range(sig_ncols):
            sig_matrix[k, col] = np.where(temp[:, col] == 1)[0][0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-hf", "--hashfile", type=str, help="Specify the complete CSV/txt file path to output hash values.")
